...
pca_data <- select(pca_data, c(Group, PC1, PC2))
...
## Use the top 500 variables by variance as is done using the deseq2 pca function
p <- get_pca_obj(rlog_df, meta, 0.75)
## Use the top 500 variables by variance as is done using the deseq2 pca function
analyzeRNA::pca_analysis(rlog_df, plot_aes, meta, varianceRemove = 0.75, draw_ellipses = FALSE)
screeplot(p)
...
# PERMANOVA for testing if distributions are different
# distance matrix for pca data
sum_squares_calc <- function(dat, vars) {
    n <- nrow(dat)
    dat <- dplyr::select(dat, any_of(vars))
    dist <- dist(dat, diag = TRUE, method = "manhattan")
    dist_sq <- dist^2
    return(sum(dist_sq) / n)
}
pc_dist <- dist(select(pca_data, c("PC1", "PC2")))
pc_dist_ctrl <- dist(select(filter(pca_data, Group == "Control"), c("PC1", "PC2")))
pc_dist_but <- dist(select(filter(pca_data, Group == "Butyrate"), c("PC1", "PC2")))
# sum of squares
sst <- sum_squares_calc(pca_data, c("PC1", "PC2"))
# variation within group
ssw_ctrl <- sum_squares_calc(filter(pca_data, Group == "Control"), c("PC1", "PC2"))
ssw_but <- sum_squares_calc(filter(pca_data, Group == "Butyrate"), c("PC1", "PC2"))
ssw <- ssw_but + ssw_ctrl
ssb <- sst - (ssw_but + ssw_ctrl)
pseudo_f <- (ssb / (2 - 1)) / (ssw / (10 - 2))

pca_transform <- PERMANOVA::IniTransform(pca_data[, 2:3])
pca_d <- PERMANOVA::DistContinuous(pca_transform)
pca_permanova <- PERMANOVA::PERMANOVA(pca_d, pca_data$Group, PostHoc = "fdr")
pca_permanova
```
...
## Plot Gene counts
```{r gene_counts}
genecounts <- function(mgi, norm_df, metadat, plot_aes, comparisons = list(), label_samples = FALSE, overlaps_allowed = 10, yaxistozero = FALSE, pval_format = "p.format", kw_hjust = 0.2, kw_vjust = 1, yaxis = "rLog Normalized Reads") {
    norm_plotdata <- data.frame(t(norm_df[norm_df$MGI_Symbol ==
        as.character(mgi), ]))
    ...
    gene_info <- data.frame(t(dplyr::slice_head(norm_plotdata,
        n = 2
    )))
    ...
    countPlot <- ggplot2::ggplot(norm_plotdata, ggplot2::aes(
        x = Group,
        y = norm, color = Group, shape = Group, fill = Group
    )) +
        ggplot2::ggtitle(gene_info$MGI_Symbol, subtitle = gene_info$MGI_Desc) +
        ggplot2::scale_color_manual(
            labels = plot_aes$labels,
            values = plot_aes$outlineID, name = "Group: "
        ) +
        ggplot2::scale_shape_manual(
            labels = plot_aes$labels,
            values = plot_aes$shapeID, name = "Group: "
        ) +
        ggplot2::scale_fill_manual(
            labels = plot_aes$labels,
            values = plot_aes$fillID, name = "Group: "
        ) +
        ggplot2::xlab(BiocGenerics::paste("Group")) +
        ggplot2::ylab(yaxis) +
        ggplot2::scale_y_continuous(n.breaks = 8)
    if (label_samples) {
        countPlot <- countPlot + global_theme() + ggplot2::geom_point(
            size = 3.5,
            stroke = 2
        ) + ggplot2::theme(
        ...
        countPlot <- countPlot + global_theme() + ggplot2::geom_jitter(
            size = 3.5,
            width = 0.1, height = 0, stroke = 2
        ) + ggplot2::theme(
            axis.text.x = ggplot2::element_blank(),
            axis.ticks.x = ggplot2::element_blank(), panel.grid.major.y = ggplot2::element_line(
                color = "gray95",
    ...
    } else if ((yaxismin - 3) < 0) {
        countPlot <- countPlot + ggplot2::expand_limits(y = c(
            0,
            yaxismax + 3
        ))
    } else {
        countPlot <- countPlot + ggplot2::expand_limits(y = c(yaxismin -
            2, yaxismax + 1))
    }
    countPlot <- countPlot +
        ggpubr::stat_compare_means(
            comparisons = comparisons,
            vjust = 0.25, label = pval_format, # label.y.npc = "bottom",
            size = 5 # , step.increase = 0.04
        ) +
        ggpubr::stat_compare_means(
            label.y.npc = "bottom",
            show.legend = FALSE, size = 3.75, vjust = kw_vjust, hjust = kw_hjust
        )
    return(countPlot)
}

genecounts("Nr4a1", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Adgre1", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Csf1r", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Cd68", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Ccr2", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Txn1", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Klf2", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Junb", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Lcp1", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")

genecounts("Serp1", rlog_df, meta, plot_aes, list(c("Control", "Butyrate")), kw_hjust = 1, pval_format = "p.signif")
```
